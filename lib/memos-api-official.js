...
// Memory reference keywords - used when requireExplicitMemoryReference is enabled
const MEMORY_REFERENCE_KEYWORDS = [
  "记忆", "回忆说", "回忆", "recall", "memory",
  "你说的", "你刚才", "你提到", "之前", "前面",
  "you said", "you mentioned", "previous", "earlier"
];

export function detectCorrectionIntent(message, requireExplicitMemoryReference = false) {
  ...
  if (requireExplicitMemoryReference) {
    const hasMemoryRef = MEMORY_REFERENCE_KEYWORDS.some(kw =>
      lowerMsg.includes(kw.toLowerCase())
    );
    if (!hasMemoryRef) return null;
  }
...
export function buildMemFeedbackPayload(config, messages, retrievedMemories, ctx, correctionInfo) {
  ...
  if (correctionInfo && correctionInfo.correctionMessage) {
    const sanitizedCorrection = sanitizeContent(correctionInfo.correctionMessage);
    feedback_content = `User correction: "${sanitizedCorrection}". `;
    if (correctionInfo.relatedMemory) {
      const sanitizedMemory = sanitizeContent(correctionInfo.relatedMemory);
      feedback_content += `Related memory to correct: "${sanitizedMemory}"`;
    }
  }
